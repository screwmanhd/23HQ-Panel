...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($data) {
        if ($data->getTanksDestroyedHeavy() == 0) {
            echo "<script type='text/javascript'>alert('Cannot decrement below 0');</script>";
        } else {
            $data->decrementTanksDestroyedHeavy(); // Decrements the number of times the user has led a platoon using the Data::decrementTimesLeadingPlatoon() method
        }
    }
}
