...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($data) {
        if ($data->getTimesAsLeadership() == 0) {
            echo "<script type='text/javascript'>alert('Cannot decrement below 0');</script>";
        } else {
            $data->decrementTimesAsLeadership(); // Decrements the number of times the user has led a platoon using the Data::decrementTimesLeadingPlatoon() method
        }
    }
}
