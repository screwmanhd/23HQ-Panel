...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($data) {
        if ($data->getTimesBeingTankDriver() == 0) {
            echo "<script type='text/javascript'>alert('Cannot decrement below 0');</script>";
        } else {
            $data->decrementTimesBeingTankDriver(); // Decrements the number of times the user has led a platoon using the Data::decrementTimesLeadingPlatoon() method
        }
    }
}
